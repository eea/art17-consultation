...
class Summary(ConclusionView, views.View):
    ...
    def dispatch_request(self):
        ...
        if request.method == "POST":
            home_url = url_for(
                self.summary_endpoint, period=period, subject=subject, region=region
            )
    ...
    def get_annexes(self, species, period):
        annexes_results = (
            EtcDataSpeciesRegion.query.with_entities(
                text("annex_II"), text("annex_IV"), text("annex_V"), text("priority")
            )
            .filter(
                EtcDataSpeciesRegion.subject == species,
...
class SpeciesSummary(SpeciesMixin, Summary):
    ...
    def get_context(self):
        ...
        if subject:
            speciescode_row = (
                EtcDataSpeciesRegion.query.filter_by(subject=subject)
...
summary.add_url_rule(
    "/species/summary/", view_func=SpeciesSummary.as_view("species-summary")
)
summary.add_url_rule(
    "/habitat/summary/", view_func=HabitatSummary.as_view("habitat-summary")
)
...
summary.add_url_rule(
    "/species/conc/delete/",
    view_func=ConclusionDelete.as_view("species-delete", mixin=SpeciesMixin),
)
summary.add_url_rule(
    "/habitat/conc/delete/",
    view_func=ConclusionDelete.as_view("habitat-delete", mixin=HabitatMixin),
)
