...
NATURE_OF_CHANGE_OPTIONS = [
    ...
    (
        "c1",
        "use of different methods to measure or evaluate individual "
    ...
    (
        "e",
        "less accurate data than those used in the previous reporting "
        + "or absent data",
    ),
    ("nc", "no change"),
    ("N/A", "not reported"),
...
def get_title_for_habitat_country(row):
    ...
    if row.habitattype_type_asses == 0:
        s_type = (
            row.habitattype_type_details.SpeciesType
            if row.habitattype_type_details
            else row.habitattype_type
        )
    if s_info:
...
def generate_map_url(dataset_id, category, subject, region, sensitive=False):
    ...
    if region:
        if dataset.schema == "2018":
            return "&".join(
                [map_href, field_2018 + "=" + subject, "region" + "=" + region]
            )
        else:
