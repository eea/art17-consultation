...
class MixinsCommon(object):
    ...
    def get_regions(cls, period, subject, short=False):
        ...
        regions = (
            ...
            .filter(cls.model_cls.subject == subject)
            .all()
        )
    ...
    def get_group_for_subject(cls, subject):
        record = cls.model_cls.query.filter_by(subject=subject).first()
    ...
    def get_MS(cls, subject, region, period):
        ...
        ms_qs = ms_qs.with_entities(
            cls.model_cls.eu_country_code, cls.model_cls.eu_country_code
        )
        if not region:
    ...
    def get_assessors(cls, period, group):
        ...
        assessors = (
            cls.model_manual_cls.query.join(
                cls.model_cls,
                and_(
...
class HabitatMixin(MixinsCommon):
    ...
    def get_subject_details(cls, subject, dataset_id):
        return EtcDicHdHabitat.query.filter_by(
            habcode=subject, dataset_id=dataset_id
        ).first()
    ...
    def get_progress_fields(cls, conclusion_type):
        if conclusion_type == "range":
            ...
            return (
                cls.model_manual_cls.method_area,
            ...
            return (
                cls.model_manual_cls.method_structure,
