...
def get_maps_url(which, type, code):
    ...
    filename = Path(app.config["MAPS_STATIC"]) / maps_format.format(
        which=which, type=type, code=code
    )
...
class FactSheet(MethodView):
    ...
    def get_priority(self):
        return PRIORITY_TEXT.get(
            self.assessment and self.assessment.priority, "Unknown"
        )
    ...
    def get_objects(self, period, subject):
        return self.model_cls.query.filter_by(
            dataset_id=period, subject=subject
        ).order_by(self.model_cls.region, self.model_cls.eu_country_code)

    def get_pressures(self, subject, pressure_type):
        if not self.engine:
            return []
        result = self.engine.execute(
            THREATS_QUERY.format(
                subject=subject,
    ...
    def get_context_data(self, **kwargs):
        ...
        self.assessment = self.model_cls.query.filter_by(
            subject=subject, dataset_id=period
        ).first()
        self.engine = (
            db.get_engine(app, "factsheet")
            if app.config["SQLALCHEMY_BINDS"]
            and app.config["SQLALCHEMY_BINDS"].get("factsheet")
            else None
        )
...
class SpeciesFactSheet(FactSheet, SpeciesMixin):
    ...
    def get_pdf_file_name(cls, assessment):
        file_name = "{0}/{1}".format(
            slugify(assessment.group), slugify(assessment.subject)
        )
        return file_name
...
class HabitatFactSheet(FactSheet, HabitatMixin):
    ...
    def get_pdf_file_name(cls, assessment):
        name = (
            assessment.lu_factsheets and assessment.lu_factsheets.nameheader
...
class FactSheetHeader(MethodView):
    def get_context_data(self, **kwargs):
        ...
        return {
            "period": self.assessment.dataset.name,
            "subject": subject,
            "label": label,
        }
    ...
    def get(self):
        context = self.get_context_data(**request.args)
        context["logo_url"] = app.config["PDF_URL_PREFIX"] + url_for(
            "static", filename="img/eea-logo-print.jpg"
        )
